...
var Modals = (function () {
  ...
  var root = null;
  var activePreviewTimer = 0;
  var setBuildToken = 0;

  function clearPreviewTimer() {
    if (!activePreviewTimer) return;
    clearInterval(activePreviewTimer);
    activePreviewTimer = 0;
  }
  ...
  function openExport(kind) {
    ...
    snapshotInto(pv, ar);
    clearPreviewTimer();
    activePreviewTimer = setInterval(function () {
      if (!document.body.contains(pv)) { clearPreviewTimer(); return; }
  ...
  async function openSetGenerator() {
    ...
    dl.addEventListener("click", async function () {
      ...
      if (token !== setBuildToken || !document.body.contains(dl)) { resetDownloadButton(); return; }
      var zip = await ZipWriter.buildAsync(entries, function (frac) {
      ...
      a.download = "dx-shader-set-" + MODES[P.mode].key + ".zip";
      a.click();
