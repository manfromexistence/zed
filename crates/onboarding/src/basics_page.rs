...
fn render_theme_section(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    ...
    fn render_theme_previews(
        ...
        let (theme_names, family_names): (&[&str], &[SharedString]) = match appearance {
            Appearance::Light => (&LIGHT_THEMES, &LIGHT_FAMILY_NAMES),
            Appearance::Dark => (&DARK_THEMES, &DARK_FAMILY_NAMES),
        };

        let themes = theme_names
            .iter()
            .map(|theme| theme_registry.get(theme).unwrap())
            .collect::<Vec<_>>();
    ...
    v_flex()
        ...
        .child(
            SwitchField::new(
                ...
                None::<&str>,
                Some("Help improve Dx by sending anonymous usage data".into()),
        ...
        .child(
            SwitchField::new(
                ...
                None::<&str>,
                Some(
                    "Help fix Dx by sending crash reports so we can fix critical issues fast"
                        .into(),
                ),
...
fn render_worktree_auto_trust_switch(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    ...
    let tooltip_description = "Dx can only allow services like language servers, project settings, and MCP servers to run after you mark a new project as trusted.";

    SwitchField::new(
        "onboarding-auto-trust-worktrees",
        Some("Trust All Projects By Default"),
        Some("Automatically mark all new projects as trusted to unlock all Dx features".into()),
        toggle_state,
        {
            let fs = <dyn Fs>::global(cx);
            move |&selection, _, cx| {
