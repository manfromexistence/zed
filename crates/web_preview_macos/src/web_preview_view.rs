...
#[cfg(target_os = "macos")]
use wry::{
    PageLoadEvent, Rect as WryRect, WebContext, WebView, WebViewBuilder, WebViewExtMacOS,
...
struct NativeWebPreview {
    ...
    webview: WebView,
    host: crate::macos_host::MacPreviewHost,
    target: crate::macos_host::MacHostTarget,
}
...
impl WebPreviewView {
    ...
    fn sync_native_preview_host_bounds(&mut self, window: &mut Window) {
        self.reconcile_macos_native_preview_target(window);
        ...
        *self.last_applied_bounds.borrow_mut() = Some(bounds);
    }
    ...
    fn sync_native_preview_window_activation(&mut self, window: &mut Window) {
        ...
        if should_be_visible {
            ...
            if !self.native_preview_visible.replace(true) {
                let _ = set_macos_native_preview_visible(preview, true);
            }
        } else if self.native_preview_visible.replace(false) {
            let _ = set_macos_native_preview_visible(preview, false);
        }
    ...
    fn queue_deferred_ipc_message(&mut self, message: String, cx: &mut Context<Self>) {
        if let Some(error) =
            deferred_ipc_message_capacity_error(&self.deferred_ipc_messages, message.len())
        {
            self.report_action_error_message(
    ...
    fn apply_browser_events(
        ...
        for event in events {
            match event {
                ...
                BrowserEvent::IpcMessage(message) => self.queue_deferred_ipc_message(message, cx),
    ...
    fn ensure_native_preview(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if let Err(error) = result {
            ...
            if should_retry_macos_native_preview_mount(&message) {
                self.native_mount_requested.set(true);
                self.load_state = PreviewLoadState::Ready;
    ...
    #[cfg(target_os = "macos")]
    fn reconcile_macos_native_preview_target(&mut self, window: &mut Window) {
        let target = match crate::macos_host::current_host_target(window) {
            Ok(target) => target,
            Err(error) => {
                let message = error.to_string();
                if self.native_preview.borrow().is_some()
                    && should_retry_macos_native_preview_mount(&message)
                {
                    self.teardown_macos_native_preview();
                    self.load_state = PreviewLoadState::Ready;
                    window.refresh();
                }
                return;
        ...
        let current_bounds = self.host_bounds.borrow().as_ref().copied();
        let should_be_visible = self.native_preview_visible.get();
        {
            ...
            if let Err(error) = sync_macos_native_preview_target(
                preview,
                window,
                target,
                current_bounds,
                ...
                eprintln!("[web-preview-macos] failed to retarget native preview host: {error:#}");
    ...
    #[cfg(target_os = "macos")]
    fn teardown_macos_native_preview(&mut self) {
    ...
    fn capture_and_store_screenshot(
        ...
        let mut image = {
            ...
            let preview = preview
                .as_ref()
                .ok_or_else(|| anyhow!("The native macOS web preview is not available"))?;
            preview.host.capture_image()?
        };
                ...
                move |bounds, window, _cx| {
                    *host_bounds.borrow_mut() = Some(bounds);
                    ...
                    if let Some(preview) = native_preview.borrow_mut().as_mut() {
                        ...
                        #[cfg(target_os = "macos")]
                        if last_applied_bounds.borrow().as_ref().copied() != Some(bounds) {
                            let _ = set_macos_native_preview_bounds(preview, bounds);
...
fn ipc_message_capacity_error(message_len: usize) -> Option<&'static str> {
    (message_len > MAX_WEB_PREVIEW_IPC_MESSAGE_BYTES)
        .then_some("message exceeds the Web Preview IPC byte limit")
}

fn queued_browser_ipc_capacity_error(
    queue: &[BrowserEvent],
    message_len: usize,
) -> Option<&'static str> {
    if queued_browser_ipc_message_count(queue) >= MAX_DEFERRED_WEB_PREVIEW_IPC_MESSAGES {
        return Some("too many Web Preview IPC messages are already queued");
    }

    let queued_bytes = queued_browser_ipc_message_bytes(queue);
    if queued_bytes.saturating_add(message_len) > MAX_DEFERRED_WEB_PREVIEW_IPC_BYTES {
        return Some("queued Web Preview IPC messages exceed the byte limit");
    }

    None
}

fn deferred_ipc_message_capacity_error(
    messages: &[String],
    message_len: usize,
) -> Option<&'static str> {
    if let Some(error) = ipc_message_capacity_error(message_len) {
    ...
    if messages.len() >= MAX_DEFERRED_WEB_PREVIEW_IPC_MESSAGES {
        return Some("too many Web Preview IPC messages are already deferred");
    }

    let queued_bytes = messages.iter().map(String::len).sum::<usize>();
    if queued_bytes.saturating_add(message_len) > MAX_DEFERRED_WEB_PREVIEW_IPC_BYTES {
        return Some("deferred Web Preview IPC messages exceed the byte limit");
    }
...
fn create_native_preview_for_macos_window(
    ...
    let host = crate::macos_host::MacPreviewHost::new(window, *host_bounds.borrow())?;
    let target = crate::macos_host::current_host_target(window)?;
    ...
    webview.reparent(host.ns_window_ptr())?;
...
#[cfg(target_os = "macos")]
fn should_retry_macos_native_preview_mount(message: &str) -> bool {
    message.contains("The GPUI AppKit view is not attached to an NSWindow yet")
}

#[cfg(target_os = "macos")]
fn sync_macos_native_preview_target(
    ...
    let synced_target = preview.host.sync_target(window)?;
    preview.target = synced_target;
    if let Some(bounds) = bounds {
        set_macos_native_preview_bounds(preview, bounds)?;
    }
    if visible {
        set_macos_native_preview_visible(preview, true)?;
    }
    Ok(())
}
...
#[cfg(target_os = "macos")]
fn set_macos_native_preview_visible(preview: &NativeWebPreview, visible: bool) -> Result<()> {
    if visible {
        preview.host.set_visible(true);
        preview.webview.set_visible(true)?;
    } else {
        let _ = preview.host.focus_gpui_view();
        preview.webview.set_visible(false)?;
        preview.host.set_visible(false);
    }
    Ok(())
}

#[cfg(target_os = "macos")]
fn set_macos_native_preview_bounds(
    preview: &NativeWebPreview,
    bounds: Bounds<Pixels>,
) -> Result<()> {
    preview.host.set_bounds(bounds)?;
    preview
        .webview
        .set_bounds(bounds_size_to_wry_rect(bounds.size))?;
    Ok(())
}

#[cfg(target_os = "macos")]
fn bounds_size_to_wry_rect(size: gpui::Size<Pixels>) -> WryRect {
    WryRect {
        position: Position::Logical(LogicalPosition::new(0.0, 0.0)),
        size: Size::Logical(LogicalSize::new(
            f64::from(size.width.max(Pixels::from(1.0))),
            f64::from(size.height.max(Pixels::from(1.0))),
        )),
    }
}
