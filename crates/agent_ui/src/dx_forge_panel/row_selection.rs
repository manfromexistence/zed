...
impl DxForgePanel {
    ...
    fn set_active_visible_row(&mut self, index: usize, cx: &mut Context<Self>) {
    ...
    fn select_first_visible_row(&mut self, cx: &mut Context<Self>) {
        if !self.visible_rows.is_empty() {
            self.set_active_visible_row(0, cx);
        }
    }

    fn select_last_visible_row(&mut self, cx: &mut Context<Self>) {
        if let Some(last_index) = self.visible_rows.len().checked_sub(1) {
            self.set_active_visible_row(last_index, cx);
        }
    }
    ...
    pub(super) fn select_next(&mut self, _: &SelectNext, _: &mut Window, cx: &mut Context<Self>) {
        let Some(current_index) = self.active_visible_row_index() else {
            self.select_first_visible_row(cx);
            return;
        };
        ...
        if next_index < self.visible_rows.len() {
            self.set_active_visible_row(next_index, cx);
        }
    }
    ...
    pub(super) fn select_previous(
        ...
        let Some(current_index) = self.active_visible_row_index() else {
            self.select_last_visible_row(cx);
            return;
        };
        ...
        self.set_active_visible_row(previous_index, cx);
    }

    pub(super) fn select_first(&mut self, _: &SelectFirst, _: &mut Window, cx: &mut Context<Self>) {
        self.select_first_visible_row(cx);
    }
