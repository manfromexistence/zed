...
const DEFAULT_RECEIPT_CACHE_FILE: &str = "receipt-cache.dxrc";
...
pub(super) fn receipt_cache_artifact_path(receipt_root: &Path) -> PathBuf {
    env_path(DX_RECEIPT_CACHE_ARTIFACT_ENV)
        .unwrap_or_else(|| receipt_root.join(DEFAULT_RECEIPT_CACHE_FILE))
}
