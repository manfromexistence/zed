...
pub(super) fn launch_audit_summary_rows(snapshot: &DxLaunchAuditSnapshot) -> Vec<AnyElement> {
    ...
    vec![
        ...
        metric_row("Scenario", snapshot.example_status.clone()),
        metric_row("Scenario Agents", snapshot.example_agents.clone()),
        metric_row("Scenario Tokens", snapshot.example_tokens.clone()),
        metric_row("Scenario Discovery", snapshot.example_discovery.clone()),
