...
impl GpuState {
    async fn new(window: Arc<Window>) -> Self {
        ...
        let shader = device.create_shader_module(wgpu::ShaderModuleDescriptor {
            ...
            source: wgpu::ShaderSource::Wgsl(include_str!("../src/shaders/liquid_glass.wgsl").into()),
        });
    ...
    fn create_background_texture(device: &wgpu::Device, queue: &wgpu::Queue, bg_id: usize) -> wgpu::Texture {
    ...
    fn generate_procedural_background() -> (u32, u32, Vec<u8>) {
        ...
        for y in 0..h {
            for x in 0..w {
                ...
                let r = (0.5 + 0.3 * (u * 6.28 + 0.0).sin()
                    + 0.2 * ((u * 15.0 + v * 10.0) * 1.5).cos())
                    .clamp(0.05, 0.95);
                let g = (0.4 + 0.3 * (v * 6.28 + 2.09).sin()
                    + 0.15 * ((u * 12.0 - v * 8.0) * 1.3).sin())
                    .clamp(0.05, 0.95);
                let b = (0.6 + 0.3 * ((u + v) * 3.14 + 4.18).sin()
    ...
    fn handle_keyboard_input(&mut self, delta_time: f32) {
        ...
        if !self.ui_state.mouse_control {
            if self.pressed_keys.contains(&KeyCode::KeyW) {
                self.ui_state.position[1] -= delta_time * self.velocity_multiplier * self.ui_state.velocity * self.ui_state.pixel_scale;
            ...
            if self.pressed_keys.contains(&KeyCode::KeyD) {
                self.ui_state.position[0] += delta_time * self.velocity_multiplier * self.ui_state.velocity * self.ui_state.pixel_scale;
                glass_key_pressed = true;
            }
        ...
        if self.pressed_keys.contains(&KeyCode::ArrowUp) {
            self.ui_state.camera_position[1] += delta_time * self.camera_velocity_multiplier * self.ui_state.camera_velocity;
        ...
        if self.pressed_keys.contains(&KeyCode::ArrowRight) {
            self.ui_state.camera_position[0] += delta_time * self.camera_velocity_multiplier * self.ui_state.camera_velocity;
            camera_key_pressed = true;
        }
    ...
    fn update_and_render(&mut self, window: &Window) -> Result<(), wgpu::SurfaceError> {
        ...
        ui.window("Settings")
            .size([320.0, 700.0], imgui::Condition::FirstUseEver)
            ...
            .build(|| {
                ...
                if ui.collapsing_header("Shape", imgui::TreeNodeFlags::DEFAULT_OPEN) {
                    ui.slider("Power", 1.001, 6.0, &mut self.ui_state.power_factor);
                    ui.slider("Width", 0.0, 10.0, &mut self.ui_state.width);
                    ui.slider("Height", 0.0, 10.0, &mut self.ui_state.height);
                }
                ...
                if ui.collapsing_header("Chromatic Aberration", imgui::TreeNodeFlags::DEFAULT_OPEN) {
                    ...
                    ui.slider("Aberration", 0.0, 0.02, &mut self.ui_state.chromatic_aberration);
                    let mut samples = self.ui_state.aberration_samples;
                    if ui.slider("Quality", 1, 8, &mut samples) {
                ...
                if ui.collapsing_header("Blur & Noise", imgui::TreeNodeFlags::empty()) {
                    ...
                    if ui.slider("Blur Iters", 0, 10, &mut iters) {
                        self.ui_state.blur_iterations = iters as u32;
                    }
                    ui.slider("Blur Radius", 0.0, 10.0, &mut self.ui_state.blur_radius);
                    ui.slider("Blur downscale", 0.1, 1.0, &mut self.ui_state.blur_downscale);
                    ui.slider("Noise", 0.0, 0.3, &mut self.ui_state.noise);
                }
                ...
                if ui.collapsing_header("Refraction", imgui::TreeNodeFlags::DEFAULT_OPEN) {
                    ...
                    ui.slider("f(x) Power", -1.5, 6.0, &mut self.ui_state.f_power);
                    ui.slider("a", 0.0, 5.0, &mut self.ui_state.a);
                    ui.slider("b", 0.0, 6.0, &mut self.ui_state.b);
                    ui.slider("c", 0.0, 6.0, &mut self.ui_state.c);
                    ui.slider("d", 0.0, 10.0, &mut self.ui_state.d);
                }
                ...
                if ui.collapsing_header("Glow", imgui::TreeNodeFlags::DEFAULT_OPEN) {
                    ui.slider("Glow weight", -1.0, 1.0, &mut self.ui_state.glow_weight);
                    ui.slider("Glow bias", -1.0, 1.0, &mut self.ui_state.glow_bias);
                    ui.slider("Glow edge0", -1.0, 1.0, &mut self.ui_state.glow_edge0);
                    ui.slider("Glow edge1", -1.0, 1.0, &mut self.ui_state.glow_edge1);
                }
...
impl ApplicationHandler for App {
    ...
    fn window_event(
        ...
        gpu.imgui_platform.handle_event(
            gpu.imgui.io_mut(),
            window,
            &event_for_imgui,
        );
