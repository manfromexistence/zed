  ...
  const replacements = {
    ...
    __DX_STYLE_SOURCE_APPLY_CONTRACT_JSON__:
      `{
        ...
        "css_declaration_hint_schema":"zed.dx_style.css_declaration_hint.v1",
        ...
        "required_css_declaration_hint_fields":["schema","hint_ordinal","property","property_pattern","property_match","value_contains","token_hint","generator_id","source_edit_safety"],
        "review_receipt_fields":["source_apply_session","dry_run_edit_review","runtime_validation_receipt","native_mutation_writer_preflight","source_write_readiness","native_writer_dispatch"],
    ...
    __DX_STYLE_CSS_DECLARATION_DRY_RUN_CONTRACT_JSON__:
      '{"__schema":"dx.style.css-declaration-dry-run-contract","css_declaration_hint_schema":"zed.dx_style.css_declaration_hint.v1"}',
...
const expectedEditorWriteBridgeRuntimeValidationReceiptFields = [
  "schema",
...
const expectedCssDeclarationHintFields = [
  "schema",
  "hint_ordinal",
  "property",
  "property_pattern",
...
test("DX Style grouped-class read model is source-owned and editor-facing", () => {
  ...
  assert.match(readModel, /MissingSourceRecommendation/);
  assert.match(coreMod, /GroupedClassRecommendationMatch/);
  assert.match(coreMod, /grouped_class_recommendation_match/);
  ...
  assert.match(editorWriteBridgePreflight, /runtime_validation_receipt/);
  assert.match(editorWriteBridgePreflight, /mutation_write_receipt_template/);
  assert.match(editorWriteBridgePreflight, /native_mutation_writer_preflight/);
  assert.match(editorWriteBridgePreflight, /native_writer_dispatch/);
  ...
  assert.equal(
    editorWriteBridgeFixture.runtime_validation_receipt_schema,
  ...
  assert.match(sourceApplyContract, /GROUPED_CLASS_SOURCE_APPLY_CSS_DECLARATION_HINT_SCHEMA/);
  assert.match(sourceApplyContract, /zed\.dx_style\.css_declaration_hint\.v1/);
  ...
  assert.match(sourceApplyContract, /required_css_declaration_hint_fields/);
  assert.match(sourceApplyContract, /property_match/);
  ...
  assert.match(sourceApplyContract, /native writer commit plan/);
  assert.match(sourceApplyContract, /post-write source digest verification plan/);
  assert.match(sourceApplyContract, /runtime validation receipt verification/);
  ...
  assert.match(sourceApplyContract, /runtime_validation_receipt/);
  assert.match(sourceApplyContract, /mutation_write_receipt_template/);
  assert.match(sourceApplyContract, /native_mutation_writer_preflight/);
  assert.match(sourceApplyContract, /native_writer_dispatch/);
  ...
  assert.equal(
    sourceApplyFixture.css_declaration_hint_schema,
  ...
  assert.deepEqual(
    sourceApplyFixture.required_css_declaration_hint_fields,
    expectedCssDeclarationHintFields,
  );
  assert.ok(
    sourceApplyFixture.required_editor_guards.includes(
      "reverse CSS delta preview provenance match",
  ...
  assert.ok(sourceApplyFixture.review_receipt_fields.includes("runtime_validation_receipt"));
  assert.ok(sourceApplyFixture.review_receipt_fields.includes("mutation_write_receipt_template"));
  assert.ok(sourceApplyFixture.review_receipt_fields.includes("native_mutation_writer_preflight"));
  assert.ok(sourceApplyFixture.review_receipt_fields.includes("native_writer_dispatch"));
  ...
  assert.match(groupReverseCssDelta, /existing_utility_required_properties/);
  assert.match(groupReverseCssDelta, /replacement_requires_existing_utility/);
  ...
  assert.match(groupReverseCssDelta, /ArbitraryCssPropertyValue/);
  assert.match(groupReverseCssDelta, /arbitrary_bracket_token/);
  assert.match(groupReverseCssDelta, /arbitrary_css_property_token/);
  ...
  assert.match(groupReverseCssDelta, /is_arbitrary_css_property_utility/);
  ...
  assert.match(groupReverseCssDelta, /border/);
  assert.match(groupReverseCssDelta, /container-type/);
  assert.match(groupReverseCssDelta, /container-name/);
  ...
  assert.deepEqual(groupReverseCssDeltaFixture.existing_utility_required_properties, [
    "background",
    "background-image",
    "background-size",
    "border",
  ...
  assert.ok(
    groupReverseCssDeltaFixture.supported_properties.some(
      (entry) =>
        entry.property === "border" &&
        entry.utility_prefix === "[border:" &&
        entry.value_strategy === "arbitrary_css_property_value",
    ),
  );
  assert.ok(
    groupReverseCssDeltaFixture.supported_properties.some(
  ...
  assert.ok(
    groupReverseCssDeltaFixture.supported_properties.some(
      (entry) =>
        entry.property === "container-type" &&
        entry.utility_prefix === "[container-type:" &&
        entry.value_strategy === "arbitrary_css_property_value",
    ),
  );
  assert.ok(
    groupReverseCssDeltaFixture.supported_properties.some(
      (entry) =>
        entry.property === "container-name" &&
        entry.utility_prefix === "[container-name:" &&
        entry.value_strategy === "arbitrary_css_property_value",
    ),
  );
  assert.ok(
  ...
  assert.match(cssDeclarationDryRunContract, /CSS_DECLARATION_DRY_RUN_HINT_SCHEMA/);
  assert.match(cssDeclarationDryRunContract, /zed\.dx_style\.css_declaration_hint\.v1/);
  ...
  assert.match(cssDeclarationDryRunContract, /required_css_declaration_hint_fields/);
  ...
  assert.equal(
    cssDeclarationDryRunFixture.css_declaration_hint_schema,
    "zed.dx_style.css_declaration_hint.v1",
  );
  ...
  assert.deepEqual(
    cssDeclarationDryRunFixture.required_css_declaration_hint_fields,
    expectedCssDeclarationHintFields,
  );
  assert.ok(
...
test("DX Style visual generator mirror helper reports DX Code fixture freshness", () => {
...
test("Zed Style rail keeps GPUI as the shell and Web Preview as the generator host", () => {
  ...
  assert.match(snapshot, /Style controls open in Web Preview while the sidebar keeps the native shell/);
  ...
  assert.match(rail, /Preview Bridge/);
  assert.match(rail, /Bridge available/);
  assert.doesNotMatch(snapshot, /Native Sidebar|host shell only|Style generators render in Web Preview, not hand-built GPUI controls/);
...
test("Web Preview owns the DX Style generator surface action", () => {
  ...
  assert.match(webPreviewView, /"runtime_validation_receipt": receipt\.get\("runtime_validation_receipt"\)\.cloned\(\)/);
  assert.match(webPreviewView, /"mutation_write_receipt_template": receipt\.get\("mutation_write_receipt_template"\)\.cloned\(\)/);
  assert.match(webPreviewView, /"native_mutation_writer_preflight": receipt\.get\("native_mutation_writer_preflight"\)\.cloned\(\)/);
  assert.match(webPreviewView, /"native_writer_dispatch": receipt\.get\("native_writer_dispatch"\)\.cloned\(\)/);
  ...
  assert.match(sourceApply, /DX_STYLE_CSS_DECLARATION_HINT_SCHEMA/);
  assert.match(sourceApply, /zed\.dx_style\.css_declaration_hint\.v1/);
  ...
  assert.match(sourceApply, /source-apply contract is missing supported CSS declaration hint schema/);
  ...
  assert.match(sourceApply, /source-apply contract is missing CSS declaration hint required fields/);
  ...
  assert.match(sourceApply, /"css_declaration_hint_schema": contract_css_declaration_hint_schema/);
  assert.match(sourceApply, /"required_css_declaration_hint_fields": source_apply_required_css_declaration_hint_fields\.clone\(\)/);
  ...
  assert.match(sourceApply, /source-apply contract is missing native writer commit plan guard/);
  assert.match(sourceApply, /source-apply contract is missing native writer commit plan receipt field/);
  assert.match(sourceApply, /source-apply contract is missing user apply action receipt field/);
  assert.match(sourceApply, /source-apply contract is missing native mutation writer preflight field/);
  assert.match(sourceApply, /source-apply contract is missing source-write readiness receipt field/);
  assert.match(sourceApply, /source-apply contract is missing native writer dispatch receipt field/);
  ...
  assert.match(sourceApply, /DX_STYLE_MUTATION_WRITE_RECEIPT_TEMPLATE_SCHEMA/);
  ...
  assert.match(sourceApply, /fn runtime_validation_receipt/);
  assert.match(sourceApply, /fn mutation_write_receipt_template/);
  ...
  assert.match(sourceApply, /blocked_commit_plan_incomplete/);
  assert.match(sourceApply, /expected_source_digest_after/);
  ...
  assert.match(sourceApply, /runtime_validation_receipt_unverified/);
  assert.match(sourceApply, /expected_post_write_readback_digest/);
  assert.match(sourceApply, /blocked_mutation_disabled/);
  assert.match(sourceApply, /pre_write_digest_match/);
  assert.match(sourceApply, /single_editor_transaction/);
  ...
  assert.match(sourceApply, /mutate_source_confirmed/);
  assert.match(sourceApply, /mutate_source_blocked_review_only/);
  assert.match(sourceApply, /user apply action schema is missing or invalid/);
  assert.match(sourceApply, /user apply action did not come from a click event/);
  ...
  assert.match(sourceApply, /"runtime_validation_receipt": runtime_validation_receipt/);
  assert.match(sourceApply, /"mutation_write_receipt_template": mutation_write_receipt_template/);
  assert.match(sourceApply, /"native_mutation_writer_preflight": native_mutation_writer_preflight/);
  ...
  assert.match(sourceApply, /"native_writer_dispatch": native_writer_dispatch/);
  assert.doesNotMatch(
    sourceApply,
  ...
  assert.match(sourceApply, /DX_STYLE_NATIVE_MUTATION_WRITER_PREFLIGHT_SCHEMA/);
  assert.match(sourceApply, /DX_STYLE_NATIVE_WRITER_DISPATCH_SCHEMA/);
  assert.match(sourceApply, /fn native_mutation_writer_preflight/);
  assert.match(sourceApply, /fn native_writer_dispatch/);
  assert.match(sourceApply, /fn native_writer_dispatch_refused/);
  ...
  assert.match(sourceApply, /native_writer_commit_plan_ready/);
  assert.match(sourceApply, /native_writer_commit_plan_missing/);
  assert.match(sourceApply, /native_writer_commit_plan_status/);
  ...
  assert.match(sourceApply, /mutation_write_receipt_template_status/);
  assert.match(sourceApply, /native_mutation_writer_preflight_ready/);
  assert.match(sourceApply, /native_mutation_writer_preflight_missing/);
  assert.match(sourceApply, /native_mutation_writer_preflight_status/);
  ...
  assert.match(sourceApply, /write_bridge_missing_runtime_validation_receipt_field/);
  assert.match(sourceApply, /write_bridge_missing_mutation_write_receipt_template_field/);
  ...
  assert.match(sourceApply, /write_bridge_runtime_validation_receipt_schema_missing/);
  ...
  assert.match(sourceApply, /native_writer_available/);
  assert.match(sourceApply, /required_authorization/);
  assert.match(sourceApply, /explicit_mutation_authorization/);
  ...
  assert.match(sourceApply, /write_bridge_missing_authorized_runtime_validation_proof/);
  ...
  assert.match(sourceApply, /CSS declaration dry-run contract is missing hint required fields/);
  assert.match(sourceApply, /CSS declaration dry-run contract hint schema does not match source-apply contract/);
  assert.match(sourceApply, /CSS declaration hint required fields do not match source-apply contract/);
  assert.match(sourceApply, /validate_css_declaration_hint_provenance/);
  assert.match(sourceApply, /CSS declaration hint contract requires unsupported field/);
  assert.match(sourceApply, /CSS declaration hint \{label\} does not match active context/);
  assert.match(sourceApply, /CSS declaration hint \{label\} do not match active context/);
  assert.match(sourceApply, /compare_css_hint_fixed_str/);
  assert.match(sourceApply, /compare_css_hint_string_array/);
  ...
  assert.match(sourceApply, /"css_declaration_hint_schema": css_declaration_dry_run_contract\.get\("css_declaration_hint_schema"\)\.and_then\(Value::as_str\)/);
  ...
  assert.match(sourceApply, /"required_css_declaration_hint_fields": string_array_at\(css_declaration_dry_run_contract, "\/required_css_declaration_hint_fields"\)/);
  assert.match(sourceApply, /"required_hint_field_count": css_declaration_dry_run_contract\.get\("required_css_declaration_hint_fields"\)/);
  ...
  assert.equal(
    styleSourceApplyFixture.css_declaration_hint_schema,
    "zed.dx_style.css_declaration_hint.v1",
  );
  assert.equal(
  ...
  assert.ok(
    styleSourceApplyFixture.review_receipt_fields.includes("user_apply_action"),
  );
  assert.ok(
  ...
  assert.match(surfaceScript, /const sourceApplyCssDeclarationHintSchema =/);
  assert.match(
    surfaceSourceApplySessionScript,
  ...
  assert.match(surfaceScript, /const sourceApplyRequiredCssDeclarationHintFields =/);
  assert.match(surfaceCssDeclarationDryRunScript, /const cssDeclarationDryRunSchema = cssDeclarationDryRunContract\.__schema/);
  assert.match(surfaceCssDeclarationDryRunScript, /const cssDeclarationDryRunHintSchema =/);
  assert.match(surfaceCssDeclarationDryRunScript, /const cssDeclarationDryRunRequiredHintFields =/);
  ...
  assert.match(surfaceScript, /const reverseCssDeltaPayloadLimits = \{/);
  assert.match(surfaceScript, /replacementUtilities: contractNumberLimit\(reverseCssDeltaContract, "max_replacement_utilities"\)/);
  assert.match(surfaceScript, /replacementUtilityBytes: contractNumberLimit\(reverseCssDeltaContract, "max_replacement_utility_bytes"\)/);
  assert.match(surfaceScript, /replacementSourceDeclarationBytes: contractNumberLimit\(reverseCssDeltaContract, "max_replacement_source_declaration_bytes"\)/);
  ...
  assert.match(surfaceScript, /function contractNumberLimit\(contract, field\)/);
  ...
  assert.match(surfaceScript, /reverse_css_delta_contract_missing_replacement_utility_count_limit/);
  assert.match(surfaceScript, /reverse_css_delta_contract_missing_replacement_utility_byte_limit/);
  assert.match(surfaceScript, /reverse_css_delta_contract_missing_replacement_source_declaration_limit/);
  assert.match(surfaceScript, /function reverseCssDeltaReplacementPayloadDiagnostics\(preview\)/);
  assert.match(surfaceScript, /reverse_css_delta_target_utility_exceeds_contract_limit/);
  assert.match(surfaceScript, /reverse_css_delta_replacement_utility_count_exceeds_contract_limit/);
  assert.match(surfaceScript, /reverse_css_delta_replacement_utility_non_string/);
  assert.match(surfaceScript, /reverse_css_delta_replacement_utility_empty/);
  assert.match(surfaceScript, /reverse_css_delta_replacement_utility_exceeds_contract_limit/);
  assert.match(surfaceScript, /reverse_css_delta_expected_source_declaration_exceeds_contract_limit/);
  assert.match(surfaceScript, /reverse_css_delta_replacement_source_declaration_exceeds_contract_limit/);
  ...
  assert.match(surfaceScript, /sourceApplyRequiredEditorGuards/);
  assert.match(surfaceScript, /reverseCssDeltaReplacementPolicyGuard/);
  assert.match(surfaceScript, /function replacementRequiresExistingUtility\(mapping\)/);
  assert.match(surfaceScript, /function sourceApplyContractHasGuard\(guard\)/);
  ...
  assert.match(surfaceScript, /diagnostics\.push\(\.\.\.cssDeclarationHintDiagnostics\(context\)\)/);
  ...
  assert.match(surfaceScript, /function arbitraryCssPropertyReverseDeltaToken\(property, value\)/);
  assert.match(surfaceScript, /function isArbitraryCssPropertyUtility\(utility, property\)/);
  assert.match(surfaceScript, /arbitrary_css_property_value/);
  ...
  assert.match(surfaceScript, /function userApplyActionPacket\(action, buttonId\)/);
  assert.match(surfaceScript, /schema: "zed\.web_preview\.dx_style\.user_apply_action\.v1"/);
  assert.match(surfaceScript, /explicit_user_action: true/);
  assert.match(surfaceScript, /function sourceApplyRequest\(output, userApplyAction\)/);
  ...
  assert.match(surfaceScript, /css_declaration_hint_diagnostics: cssDeclarationHintDiagnostics\(zedStyleContext\)/);
  ...
  assert.match(surfaceScript, /schema: cssDeclarationHintSchema\(\)/);
  assert.match(surfaceScript, /hint_ordinal: Number\.isInteger\(context\.css_hint_ordinal\)/);
  assert.match(surfaceScript, /function cssDeclarationHintSchema\(\)/);
  assert.match(surfaceScript, /function cssDeclarationHintDiagnostics\(context = zedStyleContext\)/);
  assert.match(surfaceScript, /source_apply_contract_missing_css_declaration_hint_fields/);
  assert.match(surfaceScript, /source_apply_contract_missing_css_declaration_hint_schema/);
  assert.match(surfaceScript, /css_declaration_hint_schema_contract_mismatch/);
  assert.match(surfaceScript, /css_declaration_hint_field_contract_mismatch/);
  assert.match(surfaceScript, /css_declaration_hint_mismatch:\$\{field\}/);
  assert.match(surfaceScript, /css_declaration_hint_value_contains_mismatch/);
  ...
  assert.match(surfaceScript, /runtime_validation_receipt:/);
  assert.match(surfaceScript, /function runtimeValidationReceiptPacket\(applyGate\)/);
  assert.match(surfaceScript, /mutation_write_receipt_template:/);
  assert.match(surfaceScript, /function mutationWriteReceiptTemplatePacket\(applyGate\)/);
  assert.match(surfaceScript, /native_mutation_writer_preflight:/);
  assert.match(surfaceScript, /function nativeMutationWriterPreflightPacket\(applyGate\)/);
  assert.match(surfaceScript, /native_writer_dispatch:/);
  assert.match(surfaceScript, /function nativeWriterDispatchPacket\(applyGate, output\)/);
  ...
  assert.match(surfaceScript, /write_bridge_missing_runtime_validation_receipt_field/);
  assert.match(surfaceScript, /write_bridge_missing_mutation_write_receipt_template_field/);
  assert.match(surfaceScript, /write_bridge_missing_native_mutation_writer_preflight_field/);
  assert.match(surfaceScript, /write_bridge_missing_user_apply_action_receipt_field/);
  assert.match(surfaceScript, /write_bridge_missing_native_writer_dispatch_receipt_field/);
  ...
  assert.match(surfaceScript, /write_bridge_required_runtime_proofs_missing/);
  assert.match(surfaceScript, /write_bridge_runtime_validation_receipt_schema_missing/);
  assert.match(surfaceScript, /write_bridge_required_runtime_validation_receipt_fields_missing/);
  assert.match(surfaceScript, /write_bridge_missing_authorized_runtime_validation_proof/);
  ...
  assert.match(surfaceScript, /runtime_validation_receipt_status: "not_performed_in_web_preview"/);
  assert.match(surfaceScript, /mutation_write_receipt_template_status: "not_performed_in_web_preview"/);
  assert.match(surfaceScript, /native_mutation_writer_preflight_status: "not_performed_in_web_preview"/);
  assert.match(surfaceScript, /native_writer_dispatch_status: "not_performed_in_web_preview"/);
  assert.match(surfaceScript, /native_writer_available: false/);
  assert.match(surfaceScript, /required_authorization: \[/);
  assert.match(surfaceScript, /explicit_mutation_authorization_missing/);
  assert.match(surfaceScript, /writer_invoked: false/);
  assert.match(surfaceScript, /native_writer_implementation: "not_available_in_web_preview"/);
  assert.match(surfaceScript, /mutation_write_receipt_runtime_implementation/);
  ...
  assert.match(surfaceScript, /const reverseDeltaPreview = reverseCssDeltaPreview\(output\)/);
  assert.match(surfaceScript, /reverse_css_delta_preview: reverseDeltaPreview/);
  assert.match(surfaceScript, /reverse_css_delta_replacement_payload_diagnostics:/);
  ...
  assert.match(surfaceScript, /group_recommendation_match/);
  assert.match(surfaceScript, /Expected recommendation/);
  assert.match(surfaceScript, /Recommendation match/);
  ...
  assert.match(surfaceScript, /groupContextUtilityPreviewInfo/);
  assert.match(surfaceScript, /group_utilities_preview_chars/);
  assert.match(surfaceScript, /group_utilities_preview_truncated/);
  ...
  assert.match(surfaceScript, /reverse_css_delta_existing_utility_required_properties/);
  assert.match(surfaceScript, /reverse_css_delta_max_replacement_utilities/);
  assert.match(surfaceScript, /reverse_css_delta_max_replacement_utility_bytes/);
  assert.match(surfaceScript, /reverse_css_delta_max_replacement_source_declaration_bytes/);
  ...
  assert.match(surfaceScript, /reverse_css_delta_replacement_payload_diagnostics/);
  assert.match(surfaceScript, /reverse_css_delta_replacement_payload_diagnostic/);
  ...
  assert.match(surfaceScript, /Replacement payload diagnostics/);
  assert.match(surfaceScript, /Replacement limit/);
  ...
  assert.match(surfaceScript, /source_apply_required_editor_guards/);
  ...
  assert.match(surfaceScript, /source_apply_css_declaration_hint_schema/);
  assert.match(surfaceScript, /source_apply_required_css_declaration_hint_fields/);
  ...
  assert.match(surfaceScript, /css_declaration_dry_run_hint_schema/);
  assert.match(surfaceScript, /css_declaration_dry_run_required_context_fields/);
  assert.match(surfaceScript, /css_declaration_dry_run_required_hint_fields/);
  ...
  assert.match(surfaceScript, /css_declaration_hint_diagnostics/);
  assert.match(surfaceScript, /css_declaration_hint_diagnostic/);
  assert.match(surfaceScript, /CSS declaration source review is gated by hint provenance mismatch/);
...
test("DX Style has a real right-dock GPUI shell", () => {
  ...
  assert.match(root, /fn dx_code_root\(\) -> PathBuf/);
  assert.match(root, /DxProjectContext::shared_fallback_root\(\)\.join\("code"\)/);
  assert.doesNotMatch(root, /DX_STYLE_ROOT|DX_ZED_ROOT|r"G:\\Dx\\style"|r"G:\\Dx\\code"/);
  ...
  assert.match(editorWriteBridge, /Generated Zed mirror/);
  assert.match(editorWriteBridge, /emergency_fail_closed/);
  assert.match(editorWriteBridge, /Emergency fail-closed/);
