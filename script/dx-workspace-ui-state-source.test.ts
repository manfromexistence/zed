...
test("agent fullscreen uses agent rails while sidebar button remains dock-scoped", () => {
  ...
  assert.match(threadView, /EditorMode::AutoHeight\s*\{\s*min_lines: 1,\s*max_lines: Some\(1\),/s);
...
test("agent rails and project badges keep compact production layout", () => {
  ...
  assert.match(dxLaunchWorkspace, /fn render_response_controller\(/);
  ...
  assert.match(launchChrome, /\.child\(render_response_controller\(&status, cx\)\)/);
  ...
  assert.match(progressRail, /\.right_0\(\)/);
